...
if __name__ == "__main__":
    ...
    precipitation_hourly_nc = netCDF4.Dataset(f'{data_path}/PISCOp_h_non-DBC_2015.nc')
    ...
    lat_index = find_nearest_index(latitudes, target_latitude)
    lon_index = find_nearest_index(longitudes, target_longitude)
    ...
    nearly_station_dates = np.arange('2015-01-01', '2016-01-01', dtype='datetime64[D]')
    ...
    nearly_station_df = pd.DataFrame({'date':nearly_station_dates, 'sum_hourly':nearly_station_precipitation_daily})

    station_df = pd.read_csv(f'{data_path}/qc00000230.txt', header=None, delimiter=r"\s+")
    station_df.columns =['year', 'month', 'day', 'prep', 'temp_min', 'temp_max']
    ...
    fig, ax = plt.subplots()
    ax = station_2015.plot(ax=ax, x='date', y='prep', c='red', label='Precipitation registered by Station La Esperanza', marker='o',linestyle='None')
    _ = nearly_station_df.plot(ax=ax, kind='line', x='date', y='sum_hourly', c='cyan', label='High-resolution gridded hourly precipitation close to Station La Esperanza', linestyle='solid')
